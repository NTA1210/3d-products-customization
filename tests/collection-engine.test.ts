...
describe('collection engine',()=>{
  it('uses the spec weighting exactly',()=>{const scored=scoreCollectionProduct(source,strong);expect(scored.breakdown.style).toBe(1);expect(scored.breakdown.material).toBe(1);expect(scored.breakdown.color).toBe(1);expect(scored.score).toBeCloseTo(.925,6)});
