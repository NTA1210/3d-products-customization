...
export function applyPresetRules(rules:PresetRule[],source:'STYLE'|'PRESET',manifest:ModelManifest,configuration:ModelConfiguration,options:ApplyOptions={}):ApplyResult{const parsed=PresetRuleSetSchema.safeParse(rules);if(!parsed.success)return{ok:false,code:'INVALID_PRESET_RULES',message:parsed.error.message};const actions=compilePresetRules(parsed.data,manifest,source);if(!actions.length)return{ok:false,code:'PRESET_NO_MATCH',message:'Preset did not match any components.'};return applyActions(actions,manifest,configuration,options);}
