...
export const ManufacturingIssueSchema=z.object({
  ...
  severity:ManufacturingSeveritySchema,
  componentIds:z.array(z.string()),
  message:z.string().min(1),
  measuredValue:z.number().optional(),
  expectedRange:z.object({min:z.number().optional(),max:z.number().optional()}).optional(),
...
function matchingComponents(manifest:ModelManifest,selector:{componentId?:string;role?:string}){
  return manifest.components.filter(component=>(!selector.componentId||component.id===selector.componentId)&&(!selector.role||component.role===selector.role));
}

function dimensionAxis(dimension:'width'|'height'|'depth'){
  return dimension.toUpperCase() as 'WIDTH'|'HEIGHT'|'DEPTH';
}
  ...
  for(const rule of input.rules){
    ...
    for(const component of matchingComponents(input.manifest,definition.selector)){
      ...
      if(definition.type==='DIMENSION_RANGE'){
        const value=state.dimensionsMm[definition.dimension];
        const below=definition.min!==undefined&&value<definition.min;
        const above=definition.max!==undefined&&value>definition.max;
        if(!below&&!above)continue;
        const target=below?definition.min:definition.max;
        const suggestedActions:EditorAction[]=[];
        const mappedAxis=input.manifest.axisMapping[definition.dimension];
        if(target!==undefined&&component.editable&&component.editableAxes[mappedAxis]&&component.scalingMode==='AXIS_SCALE'){
          suggestedActions.push({type:'SET_DIMENSION',componentId:component.id,axis:dimensionAxis(definition.dimension),valueMm:target,source:'MANUAL'});
        }
      ...
      if(definition.type==='MATERIAL_CATEGORY'&&state.materialId){
        ...
        if(material&&!definition.allowedCategories.includes(material.category)){
          issues.push({id:`${rule.id}:${component.id}:material`,ruleId:rule.id,severity:rule.severity,componentIds:[component.id],message:definition.message??`${component.name} uses a material category that is not allowed for manufacturing.`});
        }
      }
    }
  }
