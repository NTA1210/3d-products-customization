...
function nodeId(index: number) {
  return `node_${pad(index)}`;
}

function meshId(index: number) {
  return `mesh_${pad(index)}`;
}

function primitiveId(meshIndex: number, primitiveIndex: number) {
  return `${meshId(meshIndex)}_prim_${pad(primitiveIndex, 2)}`;
}

function qualityOptions() {
  return {
    textureResolutionThreshold: Number(
      process.env.ASSET_TEXTURE_RESOLUTION_WARNING_THRESHOLD ?? 4096,
    ),
    textureBytesThreshold: Number(process.env.ASSET_TEXTURE_BYTES_WARNING_THRESHOLD ?? 16777216),
    rootScaleMin: Number(process.env.ASSET_ROOT_SCALE_MIN ?? 0.001),
    rootScaleMax: Number(process.env.ASSET_ROOT_SCALE_MAX ?? 1000),
  };
}
...
function accessorBounds(position: Accessor, vertices: Set<number>): Bounds | undefined {
  ...
  if (!array || stride < 3 || vertices.size === 0) return undefined;
  const min: [number, number, number] = [Infinity, Infinity, Infinity];
  const max: [number, number, number] = [-Infinity, -Infinity, -Infinity];
  for (const vertex of vertices) {
    const offset = vertex * stride;
    for (let axis = 0; axis < 3; axis += 1) {
      const value = Number(array[offset + axis]);
...
function analyzePrimitive(
  ...
  if (!position) {
    warnings.push({
      ...
      severity: 'WARNING',
      message: 'Primitive has no POSITION attribute and cannot be used as an editable surface.',
      sourceId: sourcePrimitiveId,
    });
    return {
      id: sourcePrimitiveId,
      primitiveIndex,
      mode,
      triangleCount: 0,
      vertexCount: 0,
  ...
  if (!hasMaterial) {
    warnings.push({
      ...
      severity: 'WARNING',
      message: 'Primitive has no material assigned.',
      sourceId: sourcePrimitiveId,
    });
  }
  if (!hasUv) {
    warnings.push({
      code: 'MISSING_UV',
      severity: 'INFO',
      message: 'Primitive has no TEXCOORD_0 UV attribute.',
      sourceId: sourcePrimitiveId,
    });
  }
  ...
  if (mode !== GL_TRIANGLES) {
    warnings.push({
      code: 'NON_TRIANGLE_PRIMITIVE',
      severity: 'WARNING',
      message: `Primitive mode ${mode} is preserved for viewing but is not auto-componentized.`,
      sourceId: sourcePrimitiveId,
    });
    return {
      id: sourcePrimitiveId,
  ...
  if (!positionArray) {
    warnings.push({
      code: 'POSITION_ARRAY_UNAVAILABLE',
      severity: 'WARNING',
      message: 'Primitive POSITION data is unavailable after decoding.',
      sourceId: sourcePrimitiveId,
    });
    return {
      id: sourcePrimitiveId,
  ...
  const topology = analyzeTriangleTopology({
    positions: positionArray,
    positionStride: position.getElementSize(),
    indices: indices?.getArray() ?? null,
  });

  if (!topology.indexed) {
    warnings.push({
      code: 'NON_INDEXED_GEOMETRY',
      severity: 'INFO',
      message:
        'Primitive is non-indexed. Connectivity was reconstructed by welding coincident positions before detecting geometry regions.',
      sourceId: sourcePrimitiveId,
    });
  }
  if (topology.degenerateTriangleCount > 0) {
    warnings.push({
      ...
      severity: 'INFO',
      message: `${topology.degenerateTriangleCount} degenerate triangle(s) were detected.`,
      sourceId: sourcePrimitiveId,
    });
  }
  if (topology.invalidTriangleCount > 0) {
    warnings.push({
      code: 'INVALID_TRIANGLE_INDEX',
      severity: 'WARNING',
      message: `${topology.invalidTriangleCount} triangle(s) referenced invalid vertices and were excluded from topology candidates.`,
      sourceId: sourcePrimitiveId,
    });
  }

  const regions = topology.regions.map((region) => ({
    id: `${sourcePrimitiveId}_island_${pad(region.islandIndex, 3)}`,
    sourceMeshId,
    sourcePrimitiveId,
    islandIndex: region.islandIndex,
...
function pushDuplicateNameWarnings(
  names: Array<{ name: string; id: string; kind: string }>,
  warnings: Warning[],
) {
  const buckets = new Map<string, Array<{ id: string; kind: string }>>();
  for (const item of names) {
    const name = item.name.trim();
    if (!name) continue;
    const list = buckets.get(name) ?? [];
    list.push({ id: item.id, kind: item.kind });
  ...
  for (const [name, items] of buckets) {
    if (items.length > 1) {
      warnings.push({
        code: 'DUPLICATE_NAME',
        severity: 'INFO',
        message: `Name "${name}" is used by ${items.length} source objects. Names are display labels, not business IDs.`,
      });
    }
  }
}
...
function analyzeDocument(document: Document) {
  ...
  nodes.forEach((node, index) => {
    names.push({ name: node.getName(), id: nodeId(index), kind: 'node' });
    if (!node.getMesh() && node.listChildren().length === 0) {
      warnings.push({
        code: 'EMPTY_NODE',
        severity: 'INFO',
        message: 'Node has no mesh and no children.',
  ...
  const hasMorphTargets = meshes.some((mesh) =>
    mesh.listPrimitives().some((primitive) => primitive.listTargets().length > 0),
  );
  if (hasSkins) {
    warnings.push({
      code: 'SKINNED_GEOMETRY_PRESENT',
      severity: 'WARNING',
      message:
        'Skinned geometry is present. Automatic geometry-island componentization is disabled for animated/skinned parts; keep those parts source-aligned unless explicitly prepared.',
    });
  }
  if (hasMorphTargets) {
    warnings.push({
      ...
      severity: 'WARNING',
      message:
        'Morph targets are present. Automatic geometry-island componentization is disabled for morph-driven parts to preserve deformation behavior.',
    });
  }
  if (hasAnimations) {
    warnings.push({
      ...
      severity: 'INFO',
      message:
        'Animations are present. Editing node transforms may conflict with animation channels; animated parts require explicit preparation.',
    });
  }

  const meshAnalysis = meshes.map((mesh, meshIndex) => {
    const sourceMeshId = meshId(meshIndex);
    const sourceNodeIds = nodes
      .map((node, index) => (node.getMesh() === mesh ? nodeId(index) : undefined))
    ...
    const primitives = mesh
      ...
      .map((primitive, primitiveIndex) =>
        analyzePrimitive(
          primitive,
          sourceMeshId,
          primitiveId(meshIndex, primitiveIndex),
          primitiveIndex,
          warnings,
        ),
      );
    return {
      id: sourceMeshId,
      meshIndex,
      name: mesh.getName() || `Mesh ${meshIndex + 1}`,
      sourceNodeIds,
      primitives,
    };
  });

  const componentCandidates = [] as Array<{
    id: string;
    name: string;
    sourceNodeId: string;
    sourceMeshId: string;
    sourcePrimitiveId: string;
    regionIds: string[];
    semanticStatus: 'UNCONFIRMED';
  }>;

  nodes.forEach((node, nodeIndex) => {
    const mesh = node.getMesh();
    if (!mesh) return;
    const meshIndex = meshIndexByObject.get(mesh);
    if (meshIndex === undefined) return;
    mesh.listPrimitives().forEach((_primitive, primitiveIndex) => {
      const sourcePrimitiveId = primitiveId(meshIndex, primitiveIndex);
      const primitive = meshAnalysis[meshIndex].primitives[primitiveIndex];
      componentCandidates.push({
        id: `cmp_${nodeId(nodeIndex)}_${meshId(meshIndex)}_prim_${pad(primitiveIndex, 2)}`,
        name: node.getName() || mesh.getName() || `Component ${nodeIndex + 1}`,
        sourceNodeId: nodeId(nodeIndex),
        sourceMeshId: meshId(meshIndex),
        sourcePrimitiveId,
        regionIds: primitive?.regions.map((region) => region.id) ?? [],
        semanticStatus: 'UNCONFIRMED',
      });
    });
  });
  ...
  const triangles = meshAnalysis.reduce(
    (sum, mesh) =>
      sum + mesh.primitives.reduce((value, primitive) => value + primitive.triangleCount, 0),
    0,
  );
  const regionCount = meshAnalysis.reduce(
    (sum, mesh) => sum + mesh.primitives.reduce((value, primitive) => value + primitive.regions.length, 0),
    0,
  );
  ...
  if (meshes.length === 1) {
    warnings.push({
      ...
      severity: 'WARNING',
      message:
        'This asset contains only one source mesh. Semantic parts are not assumed; geometry regions are candidates that require preparation review.',
    });
  }
  if (meshes.length === 1 && regionCount > 1 && regionCount <= maxAutoRegions && !hasSkins && !hasMorphTargets) {
    warnings.push({
      code: 'DISCONNECTED_GEOMETRY_ISLANDS',
      severity: 'INFO',
      message: `The source mesh contains ${regionCount} disconnected geometry islands. They are safe geometry candidates for manual component preparation, not semantic labels.`,
      sourceId: meshId(0),
    });
  }
  if (meshes.length === 1 && regionCount > maxAutoRegions) {
    warnings.push({
      code: 'TOO_MANY_GEOMETRY_REGIONS',
      severity: 'WARNING',
      message: `The source mesh contains ${regionCount} disconnected geometry islands, above the safe auto-componentization limit ${maxAutoRegions}. Keep the source component intact and review/author components manually instead of creating hundreds of parts.`,
      sourceId: meshId(0),
    });
  }
  if (meshes.length === 1 && regionCount === 1) {
    warnings.push({
      code: 'SINGLE_CONTINUOUS_MESH',
      severity: 'WARNING',
      message:
        'This asset is a single continuous mesh. The editor falls back to whole-component editing; semantic sub-parts cannot be inferred safely from topology alone.',
      sourceId: meshId(0),
    });
  }
  ...
  if (triangles > threshold) {
    warnings.push({
      ...
      severity: 'WARNING',
      message: `Asset contains ${triangles} triangles, above the configured warning threshold ${threshold}.`,
    });
  }
...
async function processAsset(job: BullJob<AssetProcessingJobData>) {
  ...
  try {
    ...
    console.info(
      JSON.stringify({
        event: 'asset_analysis_completed',
        assetId,
        jobId: databaseJobId,
