import {ColorPresetSchema,ComponentVariantSchema,MaterialPresetSchema,type ColorPreset,type ComponentVariant,type MaterialPreset} from '@product3d/model-schema';
...
export async function getMaterials():Promise<MaterialPreset[]>{return json<unknown[]>(await fetch(`${root()}/materials`)).then(rows=>rows.map(row=>MaterialPresetSchema.parse(row)));}
export async function getColors():Promise<ColorPreset[]>{return json<unknown[]>(await fetch(`${root()}/colors`)).then(rows=>rows.map(row=>ColorPresetSchema.parse(row)));}
