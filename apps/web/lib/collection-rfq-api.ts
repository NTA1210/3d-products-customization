...
export type CollectionRecommendation={product:{id:string;name:string;category:string;styleTags:string[];materialTags:string[];colorFamily?:string|null;componentFeatures:string[];thumbnailUrl?:string|null};score:number;breakdown:{style:number;material:number;color:number;other:number}};
...
export function recommendCollection(projectId:string,configuration:ModelConfiguration,input:{category:string;colorFamily?:string;styleTags:string[];materialTags:string[];componentFeatures:string[];limit?:number}){
  return request<{weights:{style:number;material:number;color:number;other:number};recommendations:CollectionRecommendation[]}>(`${root()}/projects/${projectId}/collection/recommendations`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({configurationJson:configuration,...input,limit:input.limit??6})});
}
