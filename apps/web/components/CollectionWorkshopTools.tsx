...
import {listRfq,listWorkshops,prepareAndSubmitRfq,recommendCollection,recommendCollectionWithExplanation,type CollectionRecommendation,type RfqResult,type Workshop} from '../lib/collection-rfq-api';
...
export default function CollectionWorkshopTools(){
  ...
  const[busy,setBusy]=useState<'collection'|'collection-ai'|'rfq'>();
  ...
  useEffect(()=>{
    setCategory('CUSTOM');setColorFamily('');setStyleTags('');setMaterialTags('');setFeatureTags('');
    setRecommendations([]);setCollectionSummary('');setNote('');setRfqs([]);setBusy(undefined);
  ...
  const input=()=>({category,colorFamily:colorFamily||undefined,styleTags:tags(styleTags),materialTags:tags(materialTags),componentFeatures:tags(featureTags)});
  const recommend=async(explain=false)=>{
    if(!store.projectId||!store.configuration)return;setBusy(explain?'collection-ai':'collection');
    try{
      const result=explain
        ?await recommendCollectionWithExplanation(store.projectId,store.configuration,input())
        :await recommendCollection(store.projectId,store.configuration,input());
      setRecommendations(result.recommendations);setCollectionSummary(result.ai?.summary??'');
    }catch(value){error(value,explain?'Collection AI explanation failed.':'Collection recommendation failed.');}finally{setBusy(undefined);}
  };
  const submit=async()=>{
  ...
  return <div className="field-group">
    ...
    <div className="row">
      <button disabled={!ready||Boolean(busy)} onClick={()=>void recommend(false)}>{busy==='collection'?'Ranking…':'Recommend'}</button>
      <button disabled={!ready||Boolean(busy)} onClick={()=>void recommend(true)}>{busy==='collection-ai'?'Explaining…':'Recommend + AI Explain'}</button>
    </div>
    <p className="hint">Ranking luôn deterministic: Style 50% · Material 25% · Color 15% · Other 10%. AI chỉ giải thích score đã có, không được đổi thứ tự hay điểm.</p>
    {collectionSummary&&<div className="warning" data-testid="collection-ai-summary"><strong>AI explanation</strong><p>{collectionSummary}</p><div className="source-id">Ranking authority: deterministic engine</div></div>}
    {recommendations.map(item=><div className="warning" key={item.product.id}><strong>{item.product.name}</strong> · {Math.round(item.score*100)}%<div className="source-id">Style {Math.round(item.breakdown.style*100)} · Material {Math.round(item.breakdown.material*100)} · Color {Math.round(item.breakdown.color*100)} · Other {Math.round(item.breakdown.other*100)}</div>{item.aiExplanation?<p className="hint">{item.aiExplanation}</p>:null}</div>)}
