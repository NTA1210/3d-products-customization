...
function LoadedModel({url,loadStartedAt}:{url:string;loadStartedAt:number}){
  ...
  const model=<group ref={groupRef} onPointerDown={event=>{if(altNavigation||event.button!==0)return;event.stopPropagation();let object:THREE.Object3D|null=event.object;while(object&&!object.userData.__componentId)object=object.parent;const id=object?.userData.__componentId as string|undefined;if(id)select(id);}}><primitive object={prepared.scene}/></group>;
...
export default function ModelViewport(){
  ...
  return <Canvas dpr={[1,2]} gl={{powerPreference:'high-performance'}} camera={{position:[4,3,5],fov:45,near:.01,far:1000000}} shadows onContextMenu={event=>event.preventDefault()} onPointerMissed={event=>{if(event.button===0&&!useDccViewportStore.getState().altNavigation)useEditorStore.getState().select(undefined);}}><ambientLight intensity={1.25}/><directionalLight position={[4,7,5]} intensity={2.2} castShadow/><NavigationAids/><DccOrbitControls/>{assetUrl?<Suspense fallback={null}><Bounds key={assetUrl} fit clip margin={1.2}><LoadedModel url={assetUrl} loadStartedAt={loadStartedAt}/></Bounds></Suspense>:null}</Canvas>;
}
