...
export class AiController{
  ...
  async suggestions(@Req()request:AuthRequest,@Param('projectId')projectId:string,@Body()body:unknown){
    ...
    const providerInput={modelMetadata:{projectId,modelId:manifest.modelId,components:manifest.components.map(c=>({id:c.id,name:c.name,role:c.role,editable:c.editable,editableAxes:c.editableAxes,scalingMode:c.scalingMode,constraints:c.constraints,variantGroupId:c.variantGroupId,allowedMaterialCategories:c.allowedMaterialCategories}))},currentConfiguration:configuration,availableMaterialIds:[...catalog.materialIds],availableVariantIds:[...catalog.variantIds],availableStyleIds:[...catalog.styleIds],availableComponentIds:[...catalog.componentIds],customerInstructions:parsed.data.instructions??'',constraintSummary:manifest.components.map(c=>({componentId:c.id,constraints:c.constraints,editableAxes:c.editableAxes,scalingMode:c.scalingMode}))};
    const prompt=`You are the design assistant for a constrained 3D product configurator. Return only schema-conforming suggestions. Never invent component, material, variant or style IDs. Every action must use source AI. Respect editability, constraints and compatibility. The user explicitly applies suggestions later.\n\nINPUT:\n${JSON.stringify(providerInput)}`;
