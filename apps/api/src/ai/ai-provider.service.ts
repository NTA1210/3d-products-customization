  ...
  async designSuggestions(input:DesignProviderInput):Promise<AiProviderResult>{
    ...
    const response=await fetch('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({
        ...
        text:{format:{type:'json_schema',name:'product_design_suggestions',strict:true,schema:AI_DESIGN_JSON_SCHEMA}},
      }),
    });
    ...
    if(!text)throw new BadGatewayException('AI provider returned no structured text output.');
    let parsed:unknown;
    try{parsed=JSON.parse(text);}catch{throw new BadGatewayException('AI provider returned invalid JSON.');}
    return{provider:'openai',model,response:AiDesignResponseSchema.parse(parsed)};
  }
}
