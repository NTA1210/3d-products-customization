...
- Database tương thích PostgreSQL 16 (`DATABASE_URL`). Có thể dùng Supabase Postgres hoặc một nhà cung cấp PostgreSQL managed khác.
...
- Supabase project cho Auth và private Storage.
- Private Storage bucket có tên theo `SUPABASE_STORAGE_BUCKET` (mặc định `product3d`).

Ứng dụng **không** sử dụng credential MinIO/S3. Các thao tác Storage phía server dùng `SUPABASE_SECRET_KEY`; trình duyệt chỉ nhận publishable key của Supabase và signed upload/download grant có thời hạn ngắn.
...
- Không bao giờ expose `SUPABASE_SECRET_KEY` hoặc `OPENAI_API_KEY` thông qua biến `NEXT_PUBLIC_*`.
...
`storage:setup` có tính idempotent. Lệnh này tạo hoặc xác minh một Supabase Storage bucket **private** và áp dụng `MAX_ASSET_BYTES` làm giới hạn kích thước file. Supabase hỗ trợ tạo private bucket và giới hạn file-size ở cấp bucket thông qua Storage API.
...
Lưu object key trong PostgreSQL. Sinh signed URL tại thời điểm request; không lưu signed URL hết hạn làm business state.

## Thứ tự rollout database và job

1. Áp dụng Prisma migration.
...
5. Lưu manifest/project/version.
...
Worker/API phát trạng thái job và failure log. Asset analysis, render, AI request và deterministic manufacturability check còn phát structured JSON event có trường duration/outcome. Chuyển stdout/stderr tới logging platform của deployment và tạo counter/timer từ trường `event`.

Các metric hữu ích từ product spec gồm:
...
- thời gian load viewer

Repository hiện cung cấp structured log input cho một số metric này; không đóng gói sẵn vendor-specific metrics backend.

## Checklist bảo mật

- Supabase Storage bucket luôn private.
- Server secret key không đi vào client bundle.
...
Application service có thể rollback độc lập khi queue/database contract còn tương thích. Database rollback nên dùng corrective migration rõ ràng; không tự động đảo ngược destructive migration ở production. Source asset bất biến trong Supabase chủ động không bị normalization/export job ghi đè.
